...
def _build_image_augmented_input(user_text, extracted_image_text):
...
def create_session(raw_input, input_type, image_file, exam_mode):
    """Create a new learning session."""
    # Step 1: Get text from input
    if input_type == "image" and image_file:
        logger.info("Starting image input route.")
        extracted_image_text = extract_text_from_image(image_file)
        raw_text = _build_image_augmented_input(raw_input, extracted_image_text)
        logger.info("Image route extraction succeeded; continuing with text model pipeline.")
    else:
        logger.info("Starting text-only input route.")
        raw_text = raw_input
